...
class Server:
    ...
    def command_handle(self, nick, user, msg):
        msg = msg.replace('\n', '') # translates into command format
        msg_list = msg.split(':')

        if msg_list:
            if nick in self.users and user is not self.users[nick]: # user only access to /nick to reconnect
                if len(msg_list) > 1 and msg_list[0] == "nick":
                    self.users[msg_list[1]] = user
                    user.id = msg_list[1]
                    send_buf(user.socket, "#SERVER you are connected!")
            else: # full command buffer, # commands.py
    ...
    def client_send(self):
        while self.state.running:
            ...
            for nick in disconnected_users:
                ...
                self.users[nick].connected = False
                del self.users[nick]

    def ping_thread(self):
        while self.state.running:
            time.sleep(PING_FREQ)
            for nick in self.users: # check if users are connected
    ...
    def run(self):
        while self.state.running:
            try:
                ...
                c_thread.start()
            except: # when printing exception, server crashes.
                pass
